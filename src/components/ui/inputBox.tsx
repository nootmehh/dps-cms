...
export default function InputBox({
  ...
  return (
    <div className={`w-full max-w-116.5 inline-flex flex-col justify-start items-start gap-1 ${containerClassName}`}>
      ...
      <div className="self-stretch h-12 px-3 py-2.5 bg-white rounded-xl inline-flex justify-start items-center gap-3 border border-g1/30 focus-within:border-g1 focus-within:ring-2 focus-within:ring-g1/15 transition-all">
        {renderIcon(leftIcon, "#0A9863")}
        <input
          placeholder={placeholder}
          className={`w-full bg-transparent text-dark text-sm font-normal font-sans placeholder:text-dark/35 outline-none border-none ${className}`}
          {...props}
        />
        {renderIcon(rightIcon, "#0A9863")}
      </div>
    </div>
  );
}
